  ...
  () => import("@/components/dashboards/dispatcher/MissionDetailMap"),
  {
    ...
    loading: () => (
      <div className="w-full h-48 sm:h-64 flex items-center justify-center bg-slate-50 border border-slate-200 rounded-xl">
        ...
        <span className="ml-2 text-xs text-slate-500 font-medium">Đang tải bản đồ...</span>
      </div>
    ),
  }
);
  ...
  if (urls.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center gap-2 h-40 bg-slate-50 rounded-xl border border-slate-200 border-dashed">
        ...
        <p className="text-xs text-slate-400">Không có hình ảnh hiện trường</p>
      </div>
    );
  }
...
export default function MissionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: missionResponse, isLoading: isMissionLoading, error: missionError } = useMissionDetail(id);
  const { data: historyEvents, isLoading: isHistoryLoading } = useMissionHistory(id);

  const mission = missionResponse;
  const rescueTeamId = mission?.rescueTeamId ?? "";
  ...
  useEffect(() => {
    if (!mission || ["COMPLETED", "ABORTED"].includes(mission.status)) return;
  ...
  const handleAbort = async () => {
    ...
    try {
      ...
      toast.error("Thao tác hủy nhiệm vụ thất bại");
    }
  };
  ...
  const getDurationText = () => {
    if (!mission?.createdAt) return "N/A";
    const start = toLocalDate(mission.createdAt)?.getTime() ?? 0;
    const end = mission.status === "COMPLETED" || mission.status === "ABORTED"
      ? (toLocalDate(mission.endTime || mission.updateAt)?.getTime() ?? Date.now())
      : now;
  ...
  if (isMissionLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] w-full py-16">
        ...
        <p className="text-slate-500 font-bold text-sm">Đang tải chi tiết nhiệm vụ...</p>
      </div>
    );
  }
  ...
  if (missionError || !mission) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] w-full py-16 px-4">
        ...
        <h3 className="text-xl font-bold text-slate-800">Không tìm thấy nhiệm vụ</h3>
        <p className="text-slate-500 text-sm mt-2 text-center max-w-md">
          Có thể nhiệm vụ không tồn tại hoặc bạn không có quyền xem thông tin chi tiết này.
        </p>
        <Button onClick={() => router.push("/dispatcher?view=missions")} className="mt-6 bg-[#003da5] hover:bg-blue-800 text-white font-bold">
  ...
  const isFinishedOrAborted = ["COMPLETED", "ABORTED"].includes(mission.status);
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-3">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              <h1 className="text-lg md:text-xl font-extrabold text-slate-900">Chi tiết nhiệm vụ</h1>
              <Badge variant="outline" className="font-mono text-xs text-slate-500 bg-slate-50 px-2 py-0.5 border-slate-200">
                ID-{mission.id.substring(0, 8).toUpperCase()}
              </Badge>
            </div>
            <p className="text-xs text-slate-400 mt-1">
              Phân công lúc: {mission.createdAt ? new Date(mission.createdAt).toLocaleString("vi-VN") : "N/A"}
            </p>
          </div>
        </div>
          ...
          <Card className="rounded-2xl border-slate-200 shadow-sm hover:shadow-md transition-all">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-slate-50 rounded-xl border border-slate-100">
                <div>
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Thời gian bắt đầu</span>
                  <span className="text-sm font-semibold text-slate-800">
                    {mission.createdAt ? new Date(mission.createdAt).toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit" }) : "N/A"}
                  </span>
                </div>
                <div>
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Thời gian kết thúc</span>
                  <span className="text-sm font-semibold text-slate-800">
                    {mission.endTime ? new Date(mission.endTime).toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit" }) : "—"}
                  </span>
                </div>
                <div>
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Tổng thời gian</span>
                ...
                <div>
                  ...
                  <span className="text-sm font-bold text-slate-800">{dictStatus[mission.status]}</span>
                </div>
              </div>
              ...
              {!isFinishedOrAborted ? (
                <div className="pt-2 border-t border-slate-100 flex flex-wrap gap-3">
                  <div className="flex-1 min-w-[200px] flex items-center gap-2">
                    <span className="text-xs font-bold text-slate-500 whitespace-nowrap">Chuyển trạng thái:</span>
                    <div className="flex-1 flex gap-1">
                      {["ASSIGNED", "EN_ROUTE", "ON_SITE", "IN_PROGRESS"].map((st) => {
                        const isCurrent = mission.status === st;
                        return (
                          <Button
                            key={st}
                            size="sm"
                            ...
                            className={cn(
                              ...
                              isCurrent && "bg-blue-600 hover:bg-blue-700 text-white font-black"
                            )}
                          >
                            {st === "ASSIGNED" ? "Giao" : st === "EN_ROUTE" ? "Đi" : st === "ON_SITE" ? "Đến" : "Xử lý"}
                          </Button>
                        );
                      })}
                    </div>
                  ...
                  <div className="flex gap-2 w-full md:w-auto">
                    ...
                    >
                      <XCircle className="w-3.5 h-3.5 mr-1" /> Hủy nhiệm vụ
                    </Button>
                    <Button
                      onClick={handleFinish}
                      disabled={finishMissionMutation.isPending}
                ...
                <div className="pt-2 border-t border-slate-100 flex items-center gap-2 text-xs text-slate-400 italic">
                  ...
                  Nhiệm vụ đã kết thúc ở trạng thái {dictStatus[mission.status].toUpperCase()}. Không thể thao tác cập nhật trực tiếp.
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="rounded-2xl border-slate-200 shadow-sm hover:shadow-md transition-all">
            <CardHeader className="pb-3 border-b border-slate-100">
              <div className="flex justify-between items-center">
                ...
                {teamDetail && (
                  <Badge className={cn("text-[10px] uppercase font-bold border-0",
                    teamDetail.status === "AVAILABLE" ? "bg-emerald-100 text-emerald-700" :
                    teamDetail.status === "ON_MISSION" ? "bg-blue-100 text-blue-700" : "bg-slate-100 text-slate-500"
                  )}>
                    {dictTeamStatus[teamDetail.status] || teamDetail.status}
                  </Badge>
                )}
              </div>
            </CardHeader>
            <CardContent className="pt-4 space-y-5">
              {teamDetail ? (
                <div className="space-y-4">
                  ...
                  <div>
                    <h3 className="text-lg font-black text-slate-800">{teamDetail.teamName}</h3>
                    {teamDetail.description && (
                      <p className="text-xs text-slate-500 mt-1 leading-relaxed">{teamDetail.description}</p>
                    )}
                  </div>
                  ...
                  <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center font-bold text-base shrink-0">
                        {teamDetail.leader?.fullName ? teamDetail.leader.fullName.charAt(0).toUpperCase() : <User />}
                      </div>
                      <div className="overflow-hidden">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Đội trưởng chỉ huy</span>
                        <span className="text-sm font-bold text-slate-850 truncate block">{teamDetail.leader?.fullName ?? "Chưa bổ nhiệm"}</span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-1.5 text-xs text-slate-600 flex flex-col justify-center">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="truncate">{teamDetail.leader?.email ?? "Chưa có email"}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2.5 flex items-center gap-1.5">
                      ...
                      Lực lượng trong đội ({teamMembers.length} thành viên)
                    </h4>
                    
                    {teamMembers.length === 0 ? (
                      <p className="text-xs text-slate-400 italic">Đội chưa có thành viên nào tham gia.</p>
                    ) : (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {teamMembers.map((member) => (
                          <div key={member.id} className="flex items-center gap-3 p-3 bg-white border border-slate-200 rounded-xl shadow-sm hover:border-blue-200 hover:shadow-md transition-all">
                            <div className="w-9 h-9 rounded-full bg-slate-100 text-slate-600 flex items-center justify-center font-bold text-xs shrink-0 relative overflow-hidden">
                              {member.avatar ? (
                                <Image src={member.avatar} alt={member.fullName} fill className="object-cover" />
                              ) : (
                                member.fullName.charAt(0).toUpperCase()
                              )}
                            </div>
                            <div className="overflow-hidden flex-1">
                              <span className="text-xs font-extrabold text-slate-800 truncate block">{member.fullName}</span>
                              <span className="text-[10px] text-slate-400 block truncate">{member.phoneNumber || member.email}</span>
                            </div>
                            {member.id === teamDetail.leaderId && (
                              <Badge className="bg-blue-50 text-blue-600 border-blue-200 text-[8px] font-extrabold px-1.5 h-5 shrink-0 uppercase">Leader</Badge>
                            )}
                          </div>
                        ))}
                      </div>
              ...
              {mission.dispatcher ? (
                <div className="flex items-center gap-3.5">
                  <div className="w-10 h-10 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center font-bold text-base shrink-0">
                    {(mission.dispatcher.fullName || mission.dispatcher.name || "D").charAt(0).toUpperCase()}
                  </div>
                  <div className="overflow-hidden">
                    <h4 className="font-bold text-slate-850 text-sm leading-snug">
                      {mission.dispatcher.fullName || mission.dispatcher.name || "N/A"}
                    </h4>
                    <p className="text-xs text-slate-400 flex items-center gap-1.5 mt-1">
                      <Mail className="w-3.5 h-3.5 text-slate-400" />
                      {mission.dispatcher.email || "Email chưa được liên kết"}
                    </p>
                  </div>
                </div>
              ) : (
                <p className="text-xs text-slate-400 italic">Không tìm thấy thông tin điều phối viên phụ trách.</p>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="rounded-2xl border-slate-200 shadow-sm hover:shadow-md transition-all">
            ...
            <CardContent className="pt-4 space-y-4">
              {mission.request ? (
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    <Badge variant="outline" className="text-xs font-bold text-blue-600 border-blue-200 bg-blue-50/50">
                      {dictType[mission.request.emergencyType] || mission.request.emergencyType}
                    </Badge>
                    <Badge variant="outline" className={cn("text-xs font-bold border-0", priorityColor[mission.request.priority])}>
                      {dictPriority[mission.request.priority] || mission.request.priority}
                    </Badge>
                  </div>

                  {/* Description from citizen */}
                  <div className="bg-amber-50/30 border border-amber-100 p-3 rounded-xl">
                    <span className="text-[10px] font-bold text-amber-600/80 uppercase tracking-wider block mb-1">Mô tả sự cố của người dân</span>
                    <p className="text-xs text-slate-700 leading-relaxed italic whitespace-pre-wrap">
                      "{mission.request.description || "Không có mô tả chi tiết."}"
                    </p>
                  </div>

                  {/* Image Gallery */}
                  <div>
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-2">Hình ảnh hiện trường ({mediaUrls.length})</span>
                    <ImageGallery urls={mediaUrls} />
                  </div>

                  {/* Location & map */}
                  <div className="space-y-3">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Địa chỉ & Tọa độ bản đồ</span>
                    <div className="flex gap-2 items-start text-xs text-slate-700">
                      <MapPin className="w-4 h-4 text-slate-400 shrink-0 mt-0.5" />
                      <span className="leading-snug">{mission.request.location?.address ?? "Chưa rõ địa chỉ"}</span>
                    </div>

                    {mission.request.location?.latitude && mission.request.location?.longitude && (
                      <>
                        <div className="text-[11px] font-mono text-slate-500 pl-6">
                          ({mission.request.location.latitude.toFixed(5)}, {mission.request.location.longitude.toFixed(5)})
                        </div>
                ...
                <p className="text-xs text-slate-400 italic">Không thể liên kết với thông tin yêu cầu ban đầu.</p>
              )}
            </CardContent>
          </Card>
      ...
      <Card className="rounded-2xl border-slate-200 shadow-sm hover:shadow-md transition-all">
        ...
        <CardContent className="pt-6 space-y-6">
          ...
          {isHistoryLoading ? (
            ...
            <p className="text-xs text-slate-400 italic py-4 text-center">Chưa ghi nhận sự kiện nào trong lịch sử nhiệm vụ.</p>
          ) : (
            <div className="relative pl-6 border-l-2 border-slate-200 space-y-6 ml-2">
              {historyEvents.map((evt) => {
                const isSystem = !evt.changedBy;
                const authorName = evt.changedBy?.fullName || "Hệ thống";
                const createdDate = toLocalDate(evt.createdAt);
                
                // Determine icon or color for status change
                const toStatusVal = evt.toStatus;
                const statusColor = statusColors[toStatusVal]?.dot || "bg-slate-400";
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {evt.fromStatus ? (
                            <span className="text-[10px] text-slate-400 flex items-center gap-1">
                              ...
                              <Badge className={cn("text-[9px] font-black uppercase border-0", statusColors[evt.toStatus]?.bg, statusColors[evt.toStatus]?.text)}>
                                {dictStatus[evt.toStatus] || evt.toStatus}
                              </Badge>
                            </span>
                          ) : (
                            <span className="text-[10px] text-slate-400">
                              đã ghi nhận ý kiến / sự việc
                            </span>
                          )}
                        </div>
                        <span className="text-[10px] font-medium text-slate-400 flex items-center gap-1 shrink-0">
                          <Clock className="size-3" />
                          {createdDate ? createdDate.toLocaleString("vi-VN") : "N/A"}
                        </span>
                      </div>
          ...
          <form onSubmit={handleAddNote} className="pt-4 border-t border-slate-100 space-y-3">
            <div className="space-y-1.5">
              <label htmlFor="historyNote" className="text-xs font-bold text-slate-700 flex items-center gap-1.5">
                <MessageSquare className="w-4 h-4 text-blue-600" />
                Thêm ghi chú/Ý kiến chỉ đạo nhanh vào Nhật ký nhiệm vụ
              </label>
              <Textarea
                id="historyNote"
                value={historyNote}
            ...
            <div className="flex justify-end">
              <Button
                type="submit"
                disabled={addHistoryMutation.isPending || !historyNote.trim()}
                className="bg-blue-600 hover:bg-blue-700 text-white font-bold h-9 text-xs px-5 shadow-sm"
              >
                {addHistoryMutation.isPending && <Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin" />}
      ...
      <Dialog open={isAbortDialogOpen} onOpenChange={setIsAbortDialogOpen}>
        <DialogContent className="sm:max-w-md bg-white">
          <DialogHeader>
            ...
            <DialogDescription>
              Vui lòng cung cấp lý do chi tiết hủy bỏ nhiệm vụ này. Quyết định hủy nhiệm vụ sẽ lưu trữ trong lịch sử và thông báo tới đội cứu hộ.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="flex gap-3 justify-end border-t border-slate-100 pt-3">
            ...
            <Button
              onClick={handleAbort}
              disabled={abortMissionMutation.isPending || !abortNote.trim()}
              className="bg-red-600 hover:bg-red-700 text-white font-bold text-xs"
            >
              {abortMissionMutation.isPending && <Loader2 className="size-3.5 mr-1.5 animate-spin" />}
              Xác nhận hủy
            </Button>
          </div>
        </DialogContent>
