...
import type { RequestDetail, EmergencyCategory, RequestPriority } from "@/types/request";
...
function statusConfig(status: string) {
  const cfg: Record<string, { color: string; dot: string; icon: React.ReactNode }> = {
    PENDING: { color: "text-amber-600 bg-amber-50 border-amber-200", dot: "bg-amber-500 animate-pulse", icon: <Clock className="size-3" /> },
    IN_PROGRESS: { color: "text-blue-600 bg-blue-50 border-blue-200", dot: "bg-blue-500 animate-pulse", icon: <Radio className="size-3" /> },
    RESOLVED: { color: "text-emerald-600 bg-emerald-50 border-emerald-200", dot: "bg-emerald-500", icon: <CheckCircle2 className="size-3" /> },
    CLOSED: { color: "text-slate-500 bg-slate-50 border-slate-200", dot: "bg-slate-400", icon: <XCircle className="size-3" /> },
    ON_MISSION: { color: "text-violet-600 bg-violet-50 border-violet-200", dot: "bg-violet-500 animate-pulse", icon: <Radio className="size-3" /> },
  };
...
const emergencyToApi: Record<EmergencyCategory, number> = {
  FIRE: 1, FLOOD: 2, EARTHQUAKE: 3, MEDICAL: 4,
  TRAFFIC: 5, COLLAPSE: 6, LANDSLIDE: 7, OTHER: 8,
};
...
// ─── RequestCard ──────────────────────────────────────────────────────────────
function RequestCard({ request, onClick }: { request: RequestDetail; onClick: () => void }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <span className={cn("inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full border", status.color)}>
...
// ─── CreateRequestSheet ───────────────────────────────────────────────────────
function CreateRequestSheet({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  const createMutation = useCreateCitizenRequestMutation();
  const [emergencyType, setEmergencyType] = useState<EmergencyCategory>("FLOOD");
  ...
  const handleGetLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => { setIsLocating(false); toast.error("Không thể lấy vị trí hiện tại."); },
      { enableHighAccuracy: true, timeout: 8000 },
    );
  };

  const reset = () => {
    setEmergencyType("FLOOD"); setPriority("CRITICAL");
    setAddress(""); setLandmark(""); setDescription(""); setCoords("");
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!coords.trim()) { toast.error("Vui lòng lấy tọa độ GPS trước khi gửi."); return; }
    const [lat, lng] = coords.split(",").map((v) => parseFloat(v.trim()));
    if (isNaN(lat) || isNaN(lng)) { toast.error("Tọa độ không hợp lệ."); return; }
    try {
      ...
      toast.error("Không thể gửi yêu cầu", { description: getApiErrorMessage(err) });
    }
  };

  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="left" className="w-[400px] sm:w-[460px] p-0 flex flex-col">
        ...
        <ScrollArea className="flex-1">
          <form id="create-request-form" onSubmit={handleSubmit} className="flex flex-col gap-4 p-5">
            {/* Emergency Type */}
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-semibold text-slate-700">Loại sự cố *</label>
              <Select value={emergencyType} onValueChange={(v) => setEmergencyType(v as EmergencyCategory)}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="EARTHQUAKE">🌍 Động đất</SelectItem>
                  <SelectItem value="MEDICAL">🏥 Cấp cứu y tế</SelectItem>
                  <SelectItem value="TRAFFIC">🚗 Tai nạn giao thông</SelectItem>
                  <SelectItem value="COLLAPSE">🏚️ Sập công trình</SelectItem>
                  <SelectItem value="LANDSLIDE">⛰️ Sạt lở / Thiên tai</SelectItem>
            ...
            {/* Priority */}
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-semibold text-slate-700">Mức độ khẩn cấp *</label>
              <Select value={priority} onValueChange={(v) => setPriority(v as RequestPriority)}>
                <SelectTrigger className="h-10 rounded-lg bg-slate-50 border-slate-200">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="CRITICAL">🔴 CRITICAL — Cực kỳ khẩn cấp</SelectItem>
            ...
            {/* GPS Coordinates */}
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-semibold text-slate-700">Tọa độ GPS *</label>
              ...
              <p className="text-[11px] text-slate-400">Nhấn nút định vị để tự động lấy tọa độ GPS</p>
            </div>
            ...
            {/* Landmark */}
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-semibold text-slate-700">Mốc nhận diện</label>
              <Input
                value={landmark}
            ...
            {/* Description */}
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-semibold text-slate-700">Mô tả tình trạng *</label>
              <Textarea
                value={description}
        ...
        <div className="shrink-0 px-5 py-4 border-t bg-white flex gap-2">
          ...
          >
            {createMutation.isPending
              ? <><Loader2 className="size-4 mr-1.5 animate-spin" />Đang gửi...</>
              : <><Radio className="size-4 mr-1.5" />Gửi SOS</>
            }
          </Button>
        </div>
      </SheetContent>
...
export function CitizenSidebar() {
  const { data, isLoading, isError, refetch, isFetching } = useCitizenRequestsQuery();
    ...
    if (r.location?.latitude && r.location?.longitude) {
      window.dispatchEvent(
        new CustomEvent("MOVE_MAP", {
          detail: { lat: r.location.latitude, lng: r.location.longitude },
        }),
      );
    }
  };
  ...
  return (
    <>
      <Sidebar collapsible="none" className="w-[300px] border-r border-slate-200 flex flex-col shrink-0">
        {/* ── HEADER ── */}
            ...
            <div className="flex flex-col gap-2 p-3">
              {isLoading && (
                <>
                  {Array.from({ length: 4 }).map((_, i) => (
                    <div key={i} className="flex flex-col gap-2 p-3 rounded-lg border border-slate-100">
              ...
              {isError && (
                <div className="flex flex-col items-center gap-2 py-10 text-slate-400">
                  ...
                  <p className="text-xs text-center">Không thể tải yêu cầu.<br />Vui lòng thử lại.</p>
        ...
        <SidebarFooter className="p-3 border-t border-slate-200 bg-white shrink-0">
          ...
          <div className="flex items-center justify-center gap-3 mt-2 text-[10px] text-slate-400 font-mono">
            <span>{requests.filter(r => r.status === "PENDING").length} chờ xử lý</span>
            <span>·</span>
            <span>{requests.filter(r => r.status === "IN_PROGRESS").length} đang ứng cứu</span>
          </div>
        </SidebarFooter>
