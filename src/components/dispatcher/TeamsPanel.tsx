...
import { useTeamMembersQuery } from "@/lib/api/dispatcher-queries";
import type { RescueTeamSummary } from "@/lib/api/types";
...
function TeamListView({
  ...
  const active = teams.filter(
    (t) => t.status === "ACTIVE" || t.status === "ON_DUTY" || t.status === "AVAILABLE"
  ).length;
