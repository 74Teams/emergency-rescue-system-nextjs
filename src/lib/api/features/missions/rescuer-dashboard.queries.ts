...
import {
    normalizeMission,
    normalizeRequest,
} from '../requests/dispatcher.queries'
...
export async function fetchRescuerDashboardData(): Promise<RescuerDashboardData> {
    ...
    if (membership) {
        const missionsResponse = await rescueTeamsApi.missions(
            membership.teamId
        )
        ...
        if (activeSummary?.requestId) {
            const requestResponse = await requestsApi.detail(
                activeSummary.requestId
            )
