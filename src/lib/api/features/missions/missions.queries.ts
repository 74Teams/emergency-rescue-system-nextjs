...
export const missionKeys = {
  ...
  list: (params: Record<string, any>) => [...missionKeys.lists(), params] as const,
...
export function useMissions(params: PaginationQuery & { status?: string } = {}) {
  return useQuery({
...
export function useUpdateMissionStatus() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ missionId, payload }: { missionId: string; payload: UpdateMissionStatusInput }) => 
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: missionKeys.history(variables.missionId) });
    },
  });
}
